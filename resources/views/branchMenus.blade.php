        ...
        <div class="flex justify-center">
            <div class="lg:w-2/3 md:w-3/4 sm:w-11/12 p-4">
                <div class="flex flex-wrap gap-4">
                    <!-- Cards de platos recomendados -->
                    @foreach($menu->dishes->whereIn('id', [1, 3, 5]) as $dish)
                    <div class="w-full sm:w-2/4 md:w-1/2 lg:w-2/4 max-w-md mx-auto">
                        <div class="rounded-lg bg-white shadow-xl overflow-hidden transform transition-all hover:scale-105 wow fadeInUp"
                            data-wow-duration="1.3s" data-wow-delay="0.2s">
                            <div class="max-w-md mx-auto">
                                <div class="h-[236px]"
                                    style="background-image:url('{{ asset($dish->photo) }}'); background-size:cover; background-position:center">
                                </div>
                                <div class="p-4 sm:p-6">
                                    <p class="font-bold text-gray-700 text-[22px] leading-7 mb-1">{{ $dish->name }}</p>
                                    <div class="flex flex-row">
                                        <p class="text-[#3C3C4399] text-[17px] mr-2 ">${{ $dish->price }}</p>
                                    ...
                                    <form action="{{ route('cart.add') }}" method="POST">
                                        ...
                                        <button type="submit"
                                            class="mt-4 flex items-center justify-center w-full bg-orange-500 hover:bg-orange-800 focus:ring-4 focus:outline-none focus:ring-orange-300 text-white font-medium rounded-lg text-sm px-5 py-2.5">
